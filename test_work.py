...
DT = 0.02  # base control timestep; smoothing handled separately
SWING_TIME = float(os.getenv("SPOT_SWING_TIME", "0.20"))

# Link geometry (meters) — adjust via env vars for your robot
HIP_LENGTH = float(os.getenv("SPOT_HIP_LENGTH", "0.04"))          # hip yaw offset (half body width to joint axis)
SHOULDER_LENGTH = float(os.getenv("SPOT_SHOULDER_LENGTH", "0.10"))  # femur length
LEG_LENGTH = float(os.getenv("SPOT_LEG_LENGTH", "0.10"))            # tibia length

# Servo slew-rate limit (deg per control step) for smoother motion
SERVO_MAX_DEG_PER_STEP = float(os.getenv("SERVO_MAX_DEG_PER_STEP", "3.0"))
PREV_SERVO_ANGLES = {}
SERVO_NEUTRAL_MISSING = set()
...
SERVO_NEUTRAL = {
    (k_idx, ch): NEUTRAL_ANGLES[JOINT_NAMES[(leg, j)]] + OFFSET[JOINT_NAMES[(leg, j)]]
    for (leg, j), (k_idx, ch, _, _) in MAP_JOINT_TO_SERVO.items()
}
...
def apply_servo_command(kit_idx, ch, angle, dry_run=False):
    servo_key = (kit_idx, ch)
    target = clamp(angle, SERVO_MIN, SERVO_MAX)
    if servo_key not in SERVO_NEUTRAL and servo_key not in SERVO_NEUTRAL_MISSING:
        SERVO_NEUTRAL_MISSING.add(servo_key)
        print(f"Warning: no neutral angle registered for servo {servo_key}, using target angle directly")
    last_default = SERVO_NEUTRAL.get(servo_key, target)
    last = PREV_SERVO_ANGLES.get(servo_key, last_default)
    delta = clamp(target - last, -SERVO_MAX_DEG_PER_STEP, SERVO_MAX_DEG_PER_STEP)
    limited = clamp(last + delta, SERVO_MIN, SERVO_MAX)
    PREV_SERVO_ANGLES[servo_key] = limited
    if dry_run:
        print(f"[DRY_RUN] kits[{kit_idx}].servo[{ch}].angle = {limited}")
        return
    try:
        kits[kit_idx].servo[ch].angle = limited
    except Exception as e:
        print(f"Servo error (Kit {kit_idx} Ch {ch}): {e}")
...
def main():
    ...
    tgp = BezierGait(dt=DT, Tswing=SWING_TIME)
    ...
    try:
        while True:
            ...
            StepLength *= STEP_LENGTH_SCALE
            StepVelocity *= STEP_VELOCITY_SCALE
